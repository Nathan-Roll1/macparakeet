...
- [x] Optional default-off start/stop capture cues (`playDictationCaptureSounds`) play only after capture is live and after the mic closes, never on cancel or a failed start
...
| Dictation capture sounds | Default-off Dictation setting (`playDictationCaptureSounds`). Short system cues when capture is confirmed live and after the microphone closes. Cancel/failed start stay silent. | [F1](02-features.md#f1-system-wide-dictation) |
