...
final class SettingsSearchIndexTests: XCTestCase {
    ...
    func testMeetingAILanguageQueriesFindMeetingLanguageEntry() throws {
        ...
        for query in ["meeting ai language", "follow transcript", "summary language", "output language"] {
            let ids = Set(SettingsSearchIndex.matches(query).map(\.id))
            XCTAssertTrue(ids.contains("ai.meetingLanguage"), "Query \(query) should find Meeting AI Language")
        }
    }
